...
function get (opts, cb) {
  ...
  req.responseType = opts.format || 'json';
  req.onload = function() {
  ...
  var Docs = {};
  Docs.github = function (repo, filepath, ref) {
    var url;
    if (filepath) {
      url = 'https://api.github.com/repos/' + repo + '/contents/src/' + filepath;
    } else {
      url = 'https://api.github.com/repos/' + repo + '/README.md';
    }
    if (ref) {
      url += '?ref=' + ref;
    }
    return function (callback) {
      get({url: url}, function (err, data) {
        if (err) {
          console.error('Could not fetch docs\n', err);
          callback(err);
          return;
        }
        callback(null, data);
  ...
  docPath = docPath.replace('.html', '.md');
  if (docPath[0] === '/') {
    docPath = docPath.substr(1);
  }

  Docs.github('aframevr/aframe-site', docPath)(function (err, data) {
    // TODO: Cache.
    $('#docs__main').innerHTML = marked(Base64.decode(data.content));
  });
